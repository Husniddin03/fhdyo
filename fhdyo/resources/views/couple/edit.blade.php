...
<x-app title="couple">
    <div class="flex justify-between">
        <h1 class="text-2xl font-semib$cou text-gray-900">Edit couple</h1>
        ...
        <form action="{{ route('couples.update', $couple->id) }}" method="POST"
            ...
            <div class="w-auto">
                ...
                <select required name="status" id="status"
                    ...
                    <option value="married" {{ $couple->status == 'married' ? 'selected' : '' }}>
                        Married
                    </option>
                    <option value="unmarried" {{ $couple->status == 'unmarried' ? 'selected' : '' }}>
                        Unmarried
                    </option>
                    <option value="divorced" {{ $couple->status == 'divorced' ? 'selected' : '' }}>
                        Divorced
                    </option>
                </select>
            </div>
...
<script>
    $(document).ready(function() {
        $('#husband').select2({
            placeh$couer: 'Search husband...',
            allowClear: true
        });

        $('#wife').select2({
            placeh$couer: 'Search wife...',
            allowClear: true
        });
    });
</script>
